...
import tech.alexib.yaba.kmm.android.util.shortFormat
...
private fun TransactionDetailScreen(
    ...
    val state by rememberFlowWithLifecycle(flow = viewModel.state).collectAsState(initial = TransactionDetailScreenState.Empty)
...
private fun TransactionDetailScreenContent(transaction: TransactionDetail) {
    LazyColumn(
        ...
        item {
            TransactionRow(name = transaction.merchantName?.let { "Details" } ?: "Name",
